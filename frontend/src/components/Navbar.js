      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
        ...
        <button onClick={() => setIsCartOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', cursor: 'pointer', color: '#111827', fontWeight: '600', padding: '8px 12px', borderRadius: '6px', transition: 'background 0.2s' }}>
          <div style={{ position: 'relative' }}>
            ...
            {totalItems > 0 && (
              <span style={{ position: 'absolute', top: '-6px', right: '-8px', backgroundColor: '#ef4444', color: '#fff', fontSize: '11px', fontWeight: 'bold', height: '18px', width: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%' }}>
                {totalItems}
              </span>
            )}
          </div>
